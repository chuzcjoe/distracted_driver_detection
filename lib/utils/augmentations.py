...
class Resize(object):
    def __init__(self, size=[384,216]):
        self.size = size
...
class RandomContainCrop(object):
    def __init__(self, cfg):
        self.sample_options = (
            # using entire original input image
            None,
        ...
        while True:
            ...
            for _ in range(50):
                ...
                area_boxes = ((boxes[:, 2]-boxes[:, 0]) *
                        (boxes[:, 3]-boxes[:, 1]))  # [A,B]
                if not np.array_equal(inter, area_boxes): continue
...
class SSDAugmentation(object):  
    def __init__(self, cfg, size=(300, 300), mean=(104, 117, 123),#hw
                 use_base=False, writer=None):
        assert isinstance(size, collections.Sequence)   # is list or tuple
        self.use_base = use_base  # for eval mode
        self.mean = mean
        self.size = (size[1], size[0])  #wh
        self.writer = writer
        self.epoch = 0
        self.augment = Compose([
            ...
            #RandomContainCrop(cfg), #coverlp  or templp
            RandomMirror(),
            ToPercentCoords(),
            Resize(self.size),
            SubtractMeans(self.mean)
        ])
        if cfg.DATASET.SUB_DIR in ['cover_lp', ]:
            print('using RandomContainCrop ...')
            self.augment = Compose([
                ConvertFromInts(),
                ToAbsoluteCoords(),
                PhotometricDistort(),
...
if __name__ == '__main__':
    ...
    img_path = '/home/maolei/data/coverLP_det/lianyungang_002521.jpg'
    
    # [[xmin, ymin, xmax, ymax, label_ind], ... ]
    img = cv2.imread(img_path)
    h, w, c = img.shape
    boxes = np.array([[266./w, 499./h, 400./w, 536./h, 1], ])

    ssd_aug = SSDAugmentation(None)

    transform_img, box, label = ssd_aug(img, boxes[:, :4], boxes[:, 4], None)
    
    plt.figure()
    plt.imshow(transform_img.astype(np.float))
    plt.title('label: {}'.format(0))
    plt.show()
