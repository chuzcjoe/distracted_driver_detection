...
class SSD_MIMIC(nn.Module):
    ...
    def __init__(self, phase, cfg, base):
        ...
        # self.matching = None
        self.criterion = None
        self.criterion_mimic = None
        # if self.phase == 'eval':  # TODO add to config
        #     self.detect = Detect(self.num_classes, 0, 200, 0.01, 0.45)

    def forward(self, x, map_t=None, priors=None,phase='train', match_result=None, mimic_label=None,tb_writer=None):
        sources = list()
        mimic_layers = list()
        loc = list()
        conf = list()
        ...
        if phase == 'train':
            for i in range(len(sources)):
                mimic_layers.append(self.adap[i*2+1](self.adap[i*2](sources[i])))
        ...
        if phase == 'eval':
            ...
            mimic_out = self.criterion_mimic(map_t, mimic_layers,mimic_label,priors) \
                if self.criterion_mimic is not None else None
            # output = loc, conf
        return output,mimic_out

def add_adap(base,extras):
    # Extra layers added to VGG for feature scaling
    layers = []

    layers += [nn.Conv2d(12, 96,kernel_size=1, stride=1)]
...
def multibox(base, cfg,num_priors, num_classes):
    ...
    for k, v in enumerate(cfg):  # k start from 2
        if 'p2' in v:
