...
class TEMP_LPAnnotationTransform(object):
    ...
    def __call__(self, target, width, height):
        ...
        for obj in target.iter('object'):
            ...
            if name not in ['poi_palm','poi_water','poi_phone','poi_face']: continue
...
class TEMP_LPDetection(DetDataset):
    ...
    def _pre_process(self, index):
        ...
        if xml_path is not None:
            ...
            target_remove = ET.parse(self.data_root+xml_path).getroot()
        else:
            target = None
        
        if 'http' not in img_path:
            img = cv2.imread(osp.join(self.data_root, img_path))  # Shape(H, W, C)
            for obj in target_remove.iter('object'):
                name = obj.find('name').text.lower().strip()
                bbox = obj.find('bndbox')
                if name in ['FJS_poi_face','FJS_poi_palm']:
                       pts = ['xmin','ymin','xmax','ymax']
                       bndbox = []
                       for pt in pts:
                              cur_pt = int(float(bbox.find(pt).text))
                              bndbox.append(cur_pt)
                       img[bndbox[1]:bndbox[3],bndbox[0]:bndbox[2],:] = 0
        else:
            try:
